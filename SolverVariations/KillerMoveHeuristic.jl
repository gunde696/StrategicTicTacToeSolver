...
function orderMoves!(locations::Array{UInt8}, depth)
    index = 2
    killerMove = killerMoves[depth]
    while index <= length(locations)
        if killerMove == locations[index]
            swap!(locations, index, 1)
            return
        end
        index = index + 1
    end
...
function valueOfBoardAlphaBeta(board::Tuple{UInt64, UInt64, UInt64}, lastMove::UInt8, numMoves::UInt8, alpha, beta)::Int64
    ...
    if board[1] == UInt64(18446744073709551615)
        killerMoves[numMoves-1] = lastMove
    ...
    if ((board[1] & (1 << (54 + (lastMove % 9)))) | (board[2] & (1 << (54 + (lastMove % 9)))) | (board[3] & (1 << (54 + (lastMove % 9)))) != 0)
        ...
        possibleLocation = UInt8(0)
        locations = Array{UInt8}(undef, 0)
        while possibleLocation < 81
            if (possibleLocation % 9 == 0) && (emptySpace & (UInt128(1) << (81 + (possibleLocation ÷ 9))) == (UInt128(1) << (81 + (possibleLocation ÷ 9))))
                possibleLocation = possibleLocation + 9
                continue
            end
            # If just the current spot is full, skip ahead one spot
            if emptySpace & (UInt128(1) << (possibleLocation))  == (UInt128(1) << (possibleLocation))
                possibleLocation = possibleLocation + 1
                continue
            end
            push!(locations, possibleLocation)
            possibleLocation = possibleLocation + 1
        end

        orderMoves!(locations, numMoves)
    ...
    # else, the enemy is forced to play in the corresponding section
        ...
        possibleLocation = UInt8(0)
        locations = Array{UInt8}(undef, 0)
        for possibleLocation in (lastMove % 9)*9:(lastMove % 9)*9+8
            if emptySpace & (UInt128(1) << (possibleLocation)) == (UInt128(1) << (possibleLocation))
                continue
            end
            push!(locations, possibleLocation)
...
@time print(valueOfBoardAlphaBeta(board, UInt8(56), UInt8(29), -100, 100))
