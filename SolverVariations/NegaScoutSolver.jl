...
# Determines the value of a given board using NegaScout. Fancy!
function valueOfBoardNegaScout(board::Tuple{UInt64, UInt64, UInt64}, lastMove::UInt8, numMoves::UInt8, alpha, beta)::Int64
    ...
    if ((board[1] & (1 << (54 + (lastMove % 9)))) | (board[2] & (1 << (54 + (lastMove % 9)))) | (board[3] & (1 << (54 + (lastMove % 9)))) != 0)
        ...
        # creates temporary alpha and beta values to be used in the NegaScout search
        a = alpha
        b = beta
        ...
        while location < 81
            ...
            newValue = valueOfBoardAlphaBeta(newBoard, UInt8(location), UInt8(numMoves + 1), a, b)

            # if it is O's turn, they want the minimum value.
            if board[1] & isXO == isXO

                # Checks if 'a' and 'b' need to be set to the null window
                # IE, the last search was the first OR the last search was outside the null window
                if (a == alpha) && (b == beta)
                    b = newValue
                    a = b - 1
                # Checks if the new Value is outside the null window for better. If it's for worse, it doesn't matter
                # If it is outside the window for better, then we need to re-open the window and search that location again to find it's true value.
                elseif (a >= newValue)
                    a = alpha
                    b = beta
                    continue
    ...
    # else, the enemy is forced to play in the corresponding section
        ...
        location = (lastMove % 9)*9

        while location < (lastMove % 9)*9+9
            # If just the current spot is full, skip ahead one spot
            if emptySpace & (UInt128(1) << (location)) == (UInt128(1) << (location))
                location = location + 1
                continue
...
@time print(valueOfBoardNegaScout(board, UInt8(56), UInt8(29), -100, 100))
