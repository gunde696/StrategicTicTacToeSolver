...
function isEnded(board::Tuple{UInt64, UInt64, UInt64})::Tuple{Bool, Bool}
    ...
    elseif isWonSIMD(oBoard)
        return (true, false)
    # if the logical or of the x, o, and cat boards are a full bitmask (511 = 2^9 - 1), the game is cat
...
# Determines the value of a given board using Iterative Deepening. Fancy!
function valueOfBoardIterativeDeepening(board::Tuple{UInt64, UInt64, UInt64}, lastMove::UInt8, numMoves::UInt8)::Int64
    # determines the magnitude of the nearest possible win
    maxVal = 82 - numMoves
    # Iterates over 0 to the maximum value
    for window in 0:maxVal
        # Sees if there is a win for x within a range of 1 move. That range shifts closer and closer to the end of the game as window increases
        #println("Alpha: $(maxVal - window - 1), Beta: $(maxVal - window)")
        val = valueOfBoardAlphaBeta(board, lastMove, numMoves, maxVal - window - 1, maxVal - window)
        # If that value is the true value of the game, it is returned
        if val == maxVal - window
            return val
        end
        # Same, but for o now.
        #println("Alpha: $(window - maxVal), Beta: $(window + 1 - maxVal)")
        val = valueOfBoardAlphaBeta(board, lastMove, numMoves, window - maxVal, window + 1 - maxVal)
...
# Determines the value of a given board using Alpha Beta Pruning. Fancy!
function valueOfBoardAlphaBeta(board::Tuple{UInt64, UInt64, UInt64}, lastMove::UInt8, numMoves::UInt8, alpha, beta)::Int64
    ...
    if ((board[1] & (1 << (54 + (lastMove % 9)))) | (board[2] & (1 << (54 + (lastMove % 9)))) | (board[3] & (1 << (54 + (lastMove % 9)))) != 0)
        ...
        while location < 81
            ...
            newValue = valueOfBoardAlphaBeta(newBoard, UInt8(location), UInt8(numMoves + 1), alpha, beta)
            ...
            if board[1] & isXO == isXO
                ...
                if newValue <= alpha
                    # Returns a value if the window has size 0
                    return alpha
                end
...
#board = playMany(board, (40, 43, 70, 67, 41, 47, 20, 23, 50, 51, 60, 56, 18, 0, 7, 65, 19, 16, 71, 80, 77, 49, 39, 29, 61, 69))
...
@time print(valueOfBoardIterativeDeepening(board, UInt8(69), UInt8(26)))
