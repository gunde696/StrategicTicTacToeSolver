...
# Creates the seed
seed = 5621542
# Sets the size of the transposition table, and can be used later when needing to mod the hash
transpositionTableSize = 1000000
# Creates the transposition table, fillied with -100, as that is an impossible value for the table to legitimately store
transpositionTable = fill(-100, transpositionTableSize)
# Creates the hashes to be used for Zobrist Hashing.
hashes = rand(MersenneTwister(seed), UInt128, 189)
XHashMap = hashes[1:81]
OHashMap = hashes[82:162]
XBoardHashMap = hashes[163:171]
OBoardHashMap = hashes[172:180]
CATBoardHashMap = hashes[181:189]
...
function emptySpaces(board)::UInt128
...
function play(board::Tuple{UInt64, UInt64, UInt64, UInt128}, location)::Tuple{UInt64, UInt64, UInt64, UInt128}

    # Hashes the location played
    newHash = xor(board[4], (board[1] & isXO == isXO) ? OHashMap[location + 1] : XHashMap[location + 1])
    ...
    if location < 27
        ...
        if isBoardWon((board[1] | locations[location + 1 + (board[1] & isXO == isXO ? 27 : 0)], board[2], board[3]), location)
            ...
            if isWonSIMD(UInt16(board[1] & isXO != isXO ? (subBits16(board[1], 54, 63) | (1 << UInt64(location ÷ 9))) : (subBits16(board[2], 54, 63) | (1 << UInt64(location ÷ 9)))))
                ...
                return board[1] & isXO != isXO ? (UInt64(18446744073709551615), UInt64(0), UInt64(0), newHash) : (UInt64(0), UInt64(18446744073709551615), UInt64(0), newHash)
            ...
            elseif (subBits16(board[1], 54, 63) | subBits16(board[2], 54, 63) | subBits16(board[3], 54, 63) | (1 << UInt64(location ÷ 9))) == 511
                return (UInt64(0), UInt64(0), UInt64(18446744073709551615), newHash)
            ...
            elseif board[1] & isXO != isXO
                ...
                # Hashes the newly won board
                newHash = xor(newHash, XBoardHashMap[location ÷ 9 + 1])

                return (((board[1] | locations[location + 1 + (board[1] & isXO == isXO ? 27 : 0)]) ⊻ isXO) | (1 << (54 + UInt64(location ÷ 9))), board[2], board[3], newHash)
            # this is for if o won the board
            else

                # Hashes the newly won board
                newHash = xor(newHash, OBoardHashMap[location ÷ 9 + 1])

                return (((board[1] | locations[location + 1 + (board[1] & isXO == isXO ? 27 : 0)]) ⊻ isXO), board[2] | (1 << (54 + UInt64(location ÷ 9))), board[3], newHash)
            end
        ...
        elseif (subBits16(board[1] | locations[location + 1 + (board[1] & isXO == isXO ? 27 : 0)], UInt64(location ÷ 9)*9, UInt64(location ÷ 9)*9 + 9) | subBits16(board[1] | locations[location + 1 + (board[1] & isXO == isXO ? 27 : 0)], UInt64(location ÷ 9)*9 + 27, UInt64(location ÷ 9)*9 + 36)) == 511
            ...
            # if not, then return the board as expected with modifications
                ...
                # Hashes the newly won board
                newHash = xor(newHash, CATBoardHashMap[location ÷ 9 + 1])

                return ((board[1] | locations[location + 1 + (board[1] & isXO == isXO ? 27 : 0)]) ⊻ isXO, board[2], board[3] | (1 << (54 + UInt64(location ÷ 9))), newHash)
            end
        else
            ...
            return ((board[1] | locations[location + 1 + (board[1] & isXO == isXO ? 27 : 0)]) ⊻ isXO, board[2], board[3], newHash)
        end
    ...
    elseif location < 54
        ...
        if isBoardWon((board[1], board[2] | locations[location + 1 - 27 + (board[1] & isXO == isXO ? 27 : 0)], board[3]), location)
            ...
            elseif board[1] & isXO != isXO
                newHash = xor(newHash, XBoardHashMap[location ÷ 9 + 1])
                return ((board[1] ⊻ isXO) | (1 << (54 + UInt64(location ÷ 9))), board[2] | locations[location + 1 - 27 + (board[1] & isXO == isXO ? 27 : 0)], board[3], newHash)
            else
            newHash = xor(newHash, OBoardHashMap[location ÷ 9 + 1])
                return ((board[1] ⊻ isXO), board[2] | (1 << (54 + UInt64(location ÷ 9))) | locations[location + 1 - 27 + (board[1] & isXO == isXO ? 27 : 0)], board[3], newHash)
            end
        elseif (subBits16(board[2] | locations[location + 1 - 27 + (board[1] & isXO == isXO ? 27 : 0)], UInt64(location ÷ 9)*9 - 27, UInt64(location ÷ 9)*9 - 18) | subBits16(board[2] | locations[location + 1 - 27 + (board[1] & isXO == isXO ? 27 : 0)], UInt64(location ÷ 9)*9, UInt64(location ÷ 9)*9 + 9)) == 511
            if (subBits16(board[1], 54, 63) | subBits16(board[2], 54, 63) | subBits16(board[3], 54, 63) | (1 << UInt64(location ÷ 9))) == 511
                ...
                newHash = xor(newHash, CATBoardHashMap[location ÷ 9 + 1])
                return (board[1] ⊻ isXO, board[2] | locations[location + 1 - 27 + (board[1] & isXO == isXO ? 27 : 0)], board[3] | (1 << (54 + UInt64(location ÷ 9))), newHash)
            end
        else
            return (board[1] ⊻ isXO, board[2] | locations[location + 1 - 27 + (board[1] & isXO == isXO ? 27 : 0)], board[3], newHash)
        end
    # And the third row of boards
        ...
        if isBoardWon((board[1], board[2], board[3] | locations[location + 1 - 54 + (board[1] & isXO == isXO ? 27 : 0)]), location)
            ...
            elseif board[1] & isXO != isXO
                newHash = xor(newHash, XBoardHashMap[location ÷ 9 + 1])
                return ((board[1] ⊻ isXO) | (1 << (54 + UInt64(location ÷ 9))), board[2], board[3] | locations[location + 1 - 54 + (board[1] & isXO == isXO ? 27 : 0)], newHash)
            else
                newHash = xor(newHash, OBoardHashMap[location ÷ 9 + 1])
                return ((board[1] ⊻ isXO), board[2] | (1 << (54 + UInt64(location ÷ 9))), board[3] | locations[location + 1 - 54 + (board[1] & isXO == isXO ? 27 : 0)], newHash)
            end
        elseif (subBits16(board[3] | locations[location + 1 - 54 + (board[1] & isXO == isXO ? 27 : 0)], UInt64(location ÷ 9)*9 - 54, UInt64(location ÷ 9)*9 - 45) | subBits16(board[3] | locations[location + 1 - 54 + (board[1] & isXO == isXO ? 27 : 0)], UInt64(location ÷ 9)*9 - 27, UInt64(location ÷ 9)*9 - 18)) == 511
            if (subBits16(board[1], 54, 63) | subBits16(board[2], 54, 63) | subBits16(board[3], 54, 63) | (1 << UInt64(location ÷ 9))) == 511
                ...
                newHash = xor(newHash, CATBoardHashMap[location ÷ 9 + 1])
                return (board[1] ⊻ isXO, board[2], board[3] | locations[location + 1 - 54 + (board[1] & isXO == isXO ? 27 : 0)] | (1 << (54 + UInt64(location ÷ 9))), newHash)
            end
        else
            return (board[1] ⊻ isXO, board[2], board[3] | locations[location + 1 - 54 + (board[1] & isXO == isXO ? 27 : 0)], newHash)
        end
    end
end
...
function isBoardWon(board, location)::Bool
...
function isEnded(board)::Tuple{Bool, Bool}
...
# Determines the value of a given board
function valueOfBoard(board, lastMove::UInt8, numMoves::UInt8)::Int64
    global total = total + 1

    # Zeroethly, it must be checked if this node has been explored before by finding the value of the hash in the transposition table
    possibleValue = transpositionTable[board[4] % transpositionTableSize + 1]
    if possibleValue != -100
        return possibleValue
    end

    # First, it must be checked if the board has already been won or catted, as this method is called recursively and we don't like infinite loops
    # This corresponds to X having won, as you can see in the play() method
    if board[1] == UInt64(18446744073709551615)
        # Returns a more positive number the less moves it has taken X to win in
        transpositionTable[board[4] % transpositionTableSize + 1] = 82 - numMoves
        return 82 - numMoves
    # Corresponds to O having won
    elseif board[2] == UInt64(18446744073709551615)
        # Returns a more negative number the less moves it has taken O to win in
        transpositionTable[board[4] % transpositionTableSize + 1] = numMoves - 82
        return numMoves - 82
    # Corresponds to a CAT game
    elseif board[3] == UInt64(18446744073709551615)
        # Returns 0, if you couldn't tell
        transpositionTable[board[4] % transpositionTableSize + 1] = 0
        return 0
    end

    # uses the emptySpaces method to determine the empty spaces and stores the value in emptySpace. Shocking, right?
    emptySpace = emptySpaces(board)
    ...
    if ((board[1] & (1 << (54 + (lastMove % 9)))) | (board[2] & (1 << (54 + (lastMove % 9)))) | (board[3] & (1 << (54 + (lastMove % 9)))) != 0)
        ...
        location = UInt8(0)
        # if it is O's turn, they want the minimum value.
        if board[1] & isXO == isXO
            bestValue = 82
        # otherwise, X wants the maximum value
        else
            bestValue = -82
        end

        # Loops through every possible location on the board, as the enemy can play anywhere
        while location < 81
            # If the current board about to be looped through is full, skip 9 moves ahead
            if (location % 9 == 0) && (emptySpace & (UInt128(1) << (81 + (location ÷ 9))) == (UInt128(1) << (81 + (location ÷ 9))))
                location = location + 9
                continue
            end
            ...
            # Creates a new board with the current location played on it
            newBoard = play(board, location)
            # Finds the value of said board
            newValue = valueOfBoard(newBoard, UInt8(location), UInt8(numMoves + 1))
    ...
    transpositionTable[board[4] % transpositionTableSize + 1] = bestValue
    return bestValue
end
...
print("Begin:")
@time board = (UInt64(0), UInt64(0), UInt64(0), UInt128(0))
...
# This block of plays means the game will be a cat
#=board = play(board, 63)
board = play(board, 66)
board = play(board, 64)
board = play(board, 67)=#
...
println(bitstring(emptySpaces(board)))
print("Is Ended: ")
@time isEnded(board)
print("Play: ")
@time play(board, 27)
...
@time print(valueOfBoard(board, UInt8(46), UInt8(29)))
